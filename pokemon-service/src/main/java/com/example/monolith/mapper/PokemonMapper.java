...
public interface PokemonMapper {
    ...
    PokemonDto toDto(Pokemon pokemon, Type type, PhotoDto photo);
    ...
    default List<PokemonDto> toListDto(List<Pokemon> pokemonList, List<Type> types, List<PhotoDto> photos) {
        List<PokemonDto> pokemonDtoList = pokemonList.stream().map(pokemon -> {
            ...
            pokeDto.setPhoto(photos.stream().filter(photo -> photo.getId().equals(pokemon.getPhotoId())).findFirst().orElseThrow(PhotoNotFoundException::new).getPhoto());
            return pokeDto;
